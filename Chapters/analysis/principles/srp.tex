...
\evaluatePrincipleTable{\gls{srp}}{table_srp_convergence}{ \addEvalRow{\gls{soc} &
    ...
    differences, the two principles can be regarded as practically interchangeable. Many
    examples in the Artifacts show a strong convergence between \gls{srp} and \gls{soc}.
    To name one, an Expander should be able to can perform multiple Tasks to complete the
    full instantiation of the Model. Each of those Tasks can be implemented separately
    from each other. Figure \ref{fig_handlers} illustrated some of the Tasks that are
    ...
    \addEvalRow{\gls{dvt} & \npartialConvergence & Although using \gls{srp} does not
